...
{
	Renderer::SetResolutionTarget(Vector2I(1280, 720));
	...
	backgroundObject.lock()->AddComponent<ImageScroller>(AssetPaths::Files::BACKGROUND_IMAGE_PATH, ScrollDirection::HORIZONTAL, 50.0f);
	backgroundObject.lock()->GetTransform()->Scale(Vector2(0.5f, 0.5f));

	// Create Title
	auto title = GameObject::Instantiate<GameObject>();

	title.lock()->AddComponent<Sprite>(AssetPaths::Files::TITLE_IMAGE_PATH);
	...
	auto enterCallback = MakeAudioPlayAndDestroyCallback(AssetPaths::Files::BOBBLE_POP, Vector2F::Zero, 0.5f, false, false, 1.0f);
